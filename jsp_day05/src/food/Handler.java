...
public class Handler {
	...
	public Handler() {
		if(list.isEmpty()) {
			...
			list.add(new Food(nextIdx(), "굽네치킨", "고추바사삭", "치킨", 20000));
			list.add(new Food(nextIdx(), "죠스떡볶이", "떡튀순", "분식", 14000));
			list.add(new Food(nextIdx(), "서울반점", "간짜장", "중식", 11000));
			list.add(new Food(nextIdx(), "원할머니보쌈", "족보세트", "족발", 23000));
		}
	}
	...
	public int delete(int idx) {	// 원래는 param은 문자열이지만 자연스럽게 int로 변환
	...
	public int modify(Food ob) {
		
		// idx가 일치하는 객체의 리스트 상 index를 찾아준다
		int index = -1;
		...
		list.set(index,  ob);	// 찾아낸 index에 새로 전달받은 객체를 덮어쓴다

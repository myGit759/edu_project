...
<%
	request.setCharacterEncoding("UTF-8");	// <- POST로 받았는데 한글이 깨진다면 인코딩을 검사!
	// 1) 파라미터는 request 객체에서 꺼내온다
	// 2) request의 method가 POST이기 때문에, 글자 인코딩 규칙이 지정되지 않으면 한글이 깨진다
	// 3) 파라미터를 꺼내기 전에 먼저 인코딩 규칙을 설정 해주어야 한다
...
<%
	...
	// 개발자가 만들어서 넣는 모든 유형으 ㅣ자바 객체는 attribute 로 처리한다

...
public class Movie {
	...
	String getDetail() {	// 검색에서 사용할 상세한 정보를 모두 문자열로 반환
		...
		form += "장르 : %s\n";
		form += "상영시간 : %d분\n";
		form += "개봉일자 : %s\n";	 // yyyy년 MM월 dd일 형식으로 반환해야함
		form += "평점 : %.2f\n";
		form += "---------------------\n";
		String result = String.format(form, title, director, arrToString(actors), genre, runningTime, sdf.format(openningDate), grade);
		return result;
	}
	...
	// 클래스의 모든 멤버필드를 배개변수로 전달받아서, 객체를 생성하는 생성자 자동으로 만들기
 ...
 *  배우		String[]	actors
 *  장르		String		genre
 *  러닝타임	int			runningTime
 *  개봉일자	Date		openningDate
 *  평점		double		grade
 *  
 *  2) 프로그램에서 구현할 기능
 *  - 목록
